...
function ProductList_v2({page, limit, numberOfPage, brand, brand_v2, type, min, max}) {
    ...
    const para_query_value = localStorage.getItem("para_query") && JSON.parse(localStorage.getItem("para_query"))
    ...
    useEffect(() => {
        ...
        let _route =  brand ? `brand/${brand}` : ''
        // http://localhost:4000/products/adidas/data?_page=1&_limit=2
        // axios.get(`http://localhost:4000/data?_page=${page}&_limit=${limit}`)
        // axios.get(`http://localhost:5000/shoes/${_route}?_page=${page}&_limit=${limit}&${para_query}`)
        axios.get(`http://localhost:5000/shoes/${_route}?_page=${page}&_limit=${limit}&${param}`)
        .then(res => {
    ...
    return ( 
        <div id='scrollTo' className={cx('wrapper')}>
            ...
            {
                brand ? 
                <Link to={page>1 ? `/shoes/${brand}/page${page-1}` : `/shoes/${brand}/page${page}`} 
                    onClick={previousPage}
                >
                    <FontAwesomeIcon className={cx('direction_Page')} icon={faArrowLeft} />
                </Link>
                :
                <Link to={page>1 ? `/shoes/page${page-1}` : `/shoes/page${page}`} 
                    onClick={previousPage}
                >
                    <FontAwesomeIcon className={cx('direction_Page')} icon={faArrowLeft} />
                </Link>
                ...
                numberOfPage.map((item, index) => 
                {
                    ...
                    return (
                        <Link 
                            ...
                            className={cx(['page_number', active])} key={index} to={brand ? `/shoes/${brand}/page${item}` : `/shoes/page${item}`} 
                        >
                            {item}
                        </Link>
                    )
            ...
            {
                brand ? 
                <Link to={page<numberOfPage.length ? `/shoes/${brand}/page${page+1}` : `/shoes/${brand}/page${page}`} 
                    onClick={subsequentPage}
                >
                    <FontAwesomeIcon className={cx('direction_Page')} icon={faArrowRight} />
                </Link>
                :
                <Link to={page<numberOfPage.length ? `/shoes/page${page+1}` : `/shoes/page${page}`} 
                    onClick={subsequentPage}
                >
                    <FontAwesomeIcon className={cx('direction_Page')} icon={faArrowRight} />
                </Link>
